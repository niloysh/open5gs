...
IFS='|' read -r create_headers create_body < <(http_request POST /nupf-ee/v1/ee-subscriptions "$PAYLOAD" "get-create-$RUN_ID")
...
IFS='|' read -r get_headers get_body < <(http_request GET "/nupf-ee/v1/ee-subscriptions/$SUB_ID" "" "get-fetch-$RUN_ID")
json_assert_valid "$get_body"
if ! grep -q 'HTTP/1.1 200' "$get_headers"; then
  fail "get request failed: $(tr -d '\r' < "$get_headers" | head -n 1); body=$(tr -d '\n' < "$get_body")"
fi
[ "$(json_get "$get_body" "data['subscriptionId']")" = "$SUB_ID" ] || fail "get response subscriptionId mismatch"

log "get case passed (subscription $SUB_ID)"
